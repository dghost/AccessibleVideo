...
class MetalBuffer {
    ...
    init!(arguments:MTLArgument) {
        ...
        if #available(iOS 10, *) {
            buffer.addDebugMarker(arguments.name, range: 0..<size)
        }
...
class MetalBufferArray<T:MetalBuffer> {
    ...
    init!(arguments:MTLArgument, count:Int){
        ...
        _members = (0..<count).map {
            let baseOffset:Int = size * $0
            if #available(iOS 10, *) {
                buffer.addDebugMarker(arguments.name + " " + String($0), range: baseOffset..<(baseOffset+size))
            }
            return (T.self as T.Type).init(base: self._filterBufferData.advanced(by: baseOffset), size: size, arguments: arguments)!
        }
